...
class Command(BaseCommand):
    help = (
        "Processes durable ProvisioningRequest work records (both origins — "
        "legacy Payment/Product and the Phase 7 Order/course flow, "
        "docs/TARA_INTEGRATION_PROJECT.md) that are PENDING or FAILED, "
        ...
        "request with a row lock before processing it. Distinct from "
        "run_scheduled_provisioning (which only processes the legacy "
        "SCHEDULED nightly-batch policy) — this command is the Phase 7 "
        "worker for requests created inline by checkout/webhook processing. "
