...
def test_dashboard_renders_metrics(staff_client):
    ...
    assert "Tentatives totales" in content
...
def test_dashboard_handles_empty_state(staff_client):
    ...
    assert response.status_code == 200
    content = response.content.decode()
    assert "Aucune activité récente" in content
    assert "Nouveaux contacts" in content
...
def test_dashboard_renders_failures_alert(staff_client):
    ...
    assert response.status_code == 200
    content = response.content.decode()
    assert "Attention requise : échecs récents" in content
