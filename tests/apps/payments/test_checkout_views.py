...
def test_submitted_amount_ignored(client, plan, mock_tara_success):
    ...
    client.post(checkout_url(), data=valid_payload(plan, amount="1"))
...
def test_submitted_course_ignored(client, plan, other_course, mock_tara_success):
    client.post(checkout_url(), data=valid_payload(plan, course_id=other_course.id))
    order = Order.objects.get()
...
def test_submitted_currency_ignored(client, plan, mock_tara_success):
    client.post(checkout_url(), data=valid_payload(plan, currency="USD"))
    order = Order.objects.get()
    assert order.currency == "XAF"


def test_submitted_callback_url_ignored(client, plan, mock_tara_success):
    client.post(checkout_url(), data=valid_payload(plan, return_url="https://evil.example/steal", web_hook_url="https://evil.example/hook"))
...
def test_submitted_business_id_and_api_key_ignored(client, plan, mock_tara_success):
    client.post(checkout_url(), data=valid_payload(plan, business_id="hacked-biz", api_key="stolen-key"))
    call_kwargs = mock_tara_success.create_payment_link.call_args.kwargs
    assert "business_id" not in call_kwargs
    assert "api_key" not in call_kwargs


def test_inactive_plan_id_rejected(client, plan, mock_tara_success):
    """An inactive plan is excluded from the form's queryset — submitting its id fails
    field validation, same guarantee the old URL-level get_object_or_404(is_active=True) gave."""
    plan.is_active = False
    plan.save()
    response = client.post(checkout_url(), data=valid_payload(plan))
    assert response.status_code == 200  # form re-rendered, not processed
    assert not Order.objects.exists()


def test_nonexistent_plan_id_rejected(client, plan, mock_tara_success):
    response = client.post(checkout_url(), data=valid_payload(plan, plan_id=999999))
...
def test_existing_contact_is_reused(client, plan, mock_tara_success):
    ...
    client.post(checkout_url(), data=valid_payload(plan, first_name="Different", last_name="Value"))
...
def test_email_is_normalized(client, plan, mock_tara_success):
    client.post(checkout_url(), data=valid_payload(plan, email="Buyer@EXAMPLE.com"))
...
def test_duplicate_contact_not_created_on_repeat_checkout(client, plan, mock_tara_success):
    client.post(checkout_url(), data=valid_payload(plan, idempotency_key="idem-a"))
    client.post(checkout_url(), data=valid_payload(plan, idempotency_key="idem-b"))
...
def test_duplicate_submit_same_key_returns_same_order(client, plan, mock_tara_success):
    key = str(uuid.uuid4())
    client.post(checkout_url(), data=valid_payload(plan, idempotency_key=key))
    client.post(checkout_url(), data=valid_payload(plan, idempotency_key=key))
